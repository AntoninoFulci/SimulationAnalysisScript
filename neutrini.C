...
void neutrini::Loop(){
  ...
  vector<vector<TH1D*>> energy;
	...
	vector<vector<TH1D*>> energy0_500;
  ...
  for(int j=0; j<particella.size(); j++){
    ...
    energy0_500.push_back(vector<TH1D*>());
    ...
    for(int i=0; i<surf.size(); i++){
      ...
      TH1D *h_Thetap  = new TH1D(Form("%s_theta_p_Det%d", particella[j].c_str(), surf[i]), Form("%s theta_p Det %d; #theta_{p}(#circ); Particles/EOT", particella[j].c_str(), surf[i]), 100, 0, 0);
			...
			TH1D *h_Energt_0_500  = new TH1D(Form("%s_Energy_0_500_Det%d", particella[j].c_str(), surf[i]), Form("%s energy 0-500 Det %d; E (GeV); Particles/EOT", particella[j].c_str(), surf[i]), 100, 0,500*1e-3);
      ...
      energy0_500[j]   		.push_back(h_Energt_0_500);
	...
	double ne_p(0), ne_px(0), ne_py(0), ne_pz(0), ne_r_p(0), ne_th_p(0);

  double m_r(0);
  double m_Ek(0), n_Ek(0);
  double peso_eot(0), peso_eot_neutrini(0);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //***************************************************
		...
		//Neutrini
		ne_p = m_E;
		//Neutrino elettronico momentum projections
		ne_px = ne_p * m_cx;
		ne_py = ne_p * m_cy;
		ne_pz = ne_p * m_cz;
		//Neutrino elettronico radius & theta
		ne_r_p =   sqrt(ne_px*ne_px + ne_py*ne_py + ne_pz*ne_pz);
		ne_th_p =  atan2(sqrt(ne_px*ne_px+ne_py*ne_py),ne_pz) * 180 / M_PI;
    ...
    for(int i = 0; i<surf.size(); i++){
      ...
      //for the old simulation swhere flux_ID start from 0 purt surf[i]-1
			if(m_fluxID == surf[i]-1 && (m_pid == 5 || m_pid == 6 || m_pid == 27 || m_pid == 28)){
        ...
        EvsMID[0][i]->Fill(m_E, m_motherID, peso_eot);

				th_pVSr[0][i]  -> Fill(ne_th_p, m_r, peso_eot);
				m_EVSr[0][i]   ->Fill(m_E,m_r,peso_eot);
				...
				p[0][i]      -> Fill(ne_p,peso_eot);
				px[0][i]     -> Fill(ne_px,peso_eot);
				py[0][i]     -> Fill(ne_py,peso_eot);
				pz[0][i]     -> Fill(ne_pz,peso_eot);

				r[0][i]        -> Fill(m_r,peso_eot);
				theta_p[0][i]  -> Fill(ne_th_p,peso_eot);

				energy[0][i]   -> Fill(m_E,peso_eot);
	      energyLog[0][i]   -> Fill(m_E,peso_eot);
				energy0_500[0][i]   -> Fill(m_E,peso_eot);
