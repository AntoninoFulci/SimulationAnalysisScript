...
#ifdef neutrini_cxx
neutrini::neutrini(TTree *tree) : fChain(0){
   if (tree == 0) {
...
Int_t neutrini::GetEntry(Long64_t entry){
  // Read contents of entry.
  if (!fChain) return 0;
  return fChain->GetEntry(entry);
}

Long64_t neutrini::LoadTree(Long64_t entry){
...
void neutrini::Show(Long64_t entry){
...
Int_t neutrini::Cut(Long64_t entry){
  // This function may be called from Loop.
  // returns  1 if entry is accepted.
  // returns -1 otherwise.
  return 1;
}
#endif // #ifdef neutrini_cxx
